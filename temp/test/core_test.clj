(ns cnrepl.core-test
  (:require
   ...
   [cnrepl.core :as nrepl :refer [client
                                 client-session
                                 code
                                 combine-responses
                                 connect
  ...
  (:import
     (System.Threading Thread) (System.IO FileInfo)        ;;; (java.io File Writer)
     System.Net.Sockets.SocketException))                  ;;; java.net.SocketException
...
(def-repl-test cross-transport-*out*
  (let [sid (-> session meta ::nrepl/taking-until :session)]
    (with-open [transport2 (cnrepl.core/connect :port (:port *server*)
                                               :transport-fn *transport-fn*)]
      (transport/send transport2 {:op "eval" :code "(println :foo)"
                                  "session" sid})
      (is (= [{:out (platform-newlines ":foo\n")}                                   ;DM: Added platform-newlines
              {:ns "user" :value "nil"}
              {:status #{:done}}]
...
(def-repl-test return-on-incomplete-expr
  (let [{:keys [out status value]} (clean-response (combine-responses (repl-eval session "(missing paren")))]         ;;; ) -- so editor not confused
    (is (nil? value))
    (is (= #{:done :eval-error} status))
    ;; TODO avoid regex error if there's no exception found. Can be misleading
    (is (re-seq #"ReaderException" (first (repl-values session "(-> *e Throwable->map :cause)"))))))              ;;; #"EOF while reading"
    ...
    (let [resp (message session {:op "eval"
      ...
      (let [r (->> resp
        ...
        (is (= "class java.lang.InterruptedException"
               (:ex r))))))
  #_(testing "interruptable code"                                      ;;; TODO: HOw?
    (let [resp (message session {:op "eval"
                                 :code (code
                                        (do
                                          (defn run []
...
(def-repl-test read-timeout
  (is (nil? (repl-values timeout-session "(Thread/Sleep 1100) :ok")))                ;;; sleep
  ...
  (testing "multiple messages can be handled on the same connection concurrently"
    (let [sessions (doall (repeatedly 3 #(client-session client)))
          ...
          elapsed-times (map (fn [session eval-duration]
                               (let [expr (pr-str `(Thread/Sleep ~eval-duration))            ;;; sleep
...
(def-repl-test ensure-server-closeable
  ...
  (Thread/sleep 100)
  (is (thrown? Exception (connect :port (:port *server*)))))                          ;;; java.net.ConnectException
...
(deftest server-starts-with-minimal-configuration
  (testing "Ensure server starts with minimal configuration"
    (let [server (server/start-server)
          transport (connect :port (:port server))
          client (client transport Int64/MaxValue)]                             ;;; Long/MAX_VALUE
      (is (= ["3"]
...
(def-repl-test clients-fail-on-disconnects
  (testing "Ensure that clients fail ASAP when the server they're connected to goes down."
    ...
    (try (repl-eval session "(+ 1 1)") (catch Exception _))         ;;; Throwable
    (Thread/sleep 100)
...
(def-repl-test interrupt-load-file
  (let [resp (message session {:op "load-file"
                               :file (slurp (System.IO.Path/Combine project-base-dir "load-file-test/nrepl/load_file_sample2.clj"))   ;;;File.
...
(defn custom-session-repl-caught
  [^Exception t]                                  ;;; Throwable
  (binding [*out* *err*]
    (println "bar" (.Message t))))                 ;;; .getMessage
...
#_(def sideloader-tests-lookup
  {["resource" "hello"]
   ...
   (System.IO.Path/Combine project-base-dir "test-resources/HelloFactory.class")})    ;;; File, added combine
;;; TODO: figure out what to do with sideloader
#_(defn string->content
  [str]
  (if str
    (-> str
        (.getBytes "UTF-8")
...
#_(defn message-with-sideloader
...
#_(def-repl-test sideloader-test
...
(def dynamic-test-lookup
  {["resource" "foo.clj"]
   ...
   ["resource" "ping.clj"]
   (slurp
    (System.IO.Path/Combine project-base-dir "test-resources/ping.clj"))    ;;; File.

   ["resource" "ping_imp.clj"]
   (slurp
    (System.IO.Path/Combine project-base-dir "test-resources/ping_imp.clj"))})   ;;; File.
...
#_(def-repl-test dynamic-middleware+sideloading
...
#_(def-repl-test dynamic-middleware+deferred+sideloading
  (let [rsp1 (->> (message-with-sideloader client
                                           {:op               "add-middleware"
                                            :middleware       ["ping/wrap-ping"]
...
#_(def-repl-test ^{:min-java-version "11.0"} hotloading-common-classloader-test
