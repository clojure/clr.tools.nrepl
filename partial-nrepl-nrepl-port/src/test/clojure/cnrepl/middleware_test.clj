(ns cnrepl.middleware-test
  (:require
   [clojure.test :refer [deftest is are]]
   [cnrepl.middleware :as middleware :refer [linearize-middleware-stack]]
   [cnrepl.server :refer [default-middleware]]))
...
(deftest sanity
  ...
  (let [n ^{::middleware/descriptor
        ...
        m ^{::middleware/descriptor
            {:expects #{"eval"} :requires #{n #'cnrepl.middleware.print/wrap-print}}}
        {:dummy :middleware}
...
(deftest no-descriptor-warning
  (let [^String s (with-out-str
    ...
    (is (.Contains s "No nREPL middleware descriptor in metadata of {:dummy :middleware}"))))        ;;; .contains
