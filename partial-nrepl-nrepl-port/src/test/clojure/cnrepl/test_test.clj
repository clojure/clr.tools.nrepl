...
(def tf #'transport/bencode)
(def s (server/start-server :transport-fn tf))
(def conn (connect :port (:port s) :transport-fn tf))
(def cli (cnrepl.core/client conn Int64/MaxValue))
(def sess (client-session cli))
