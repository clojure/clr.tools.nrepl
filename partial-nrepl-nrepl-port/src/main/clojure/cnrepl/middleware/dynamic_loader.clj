(ns cnrepl.middleware.dynamic-loader
  ...
  (:require [clojure.string :as str]
            [cnrepl.middleware :refer [linearize-middleware-stack set-descriptor!]]
            [cnrepl.middleware.session :as middleware.session]
            [cnrepl.misc :as misc :refer [response-for with-session-classloader]]
  ...
  (with-session-classloader session
    (run! (fn [namespace]
            (try
              ...
              (catch Exception _ nil)))                                               ;;; Throwable
          namespaces)))
